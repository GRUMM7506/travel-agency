...
```
заявка → ожидает оплаты → на проверке → оплачен → завершён
                                    ↘ отменён
```

- **ожидает оплаты** — ставится автоматически, когда клиент бронирует через `POST /bookings/self`
- **на проверке** — клиент нажал «Я оплатил»; деньги ещё не подтверждены
- **оплачен** — сотрудник увидел поступление и внёс `Payment` вручную
...
Реальные платежи (`POST /payments`) создаёт исключительно сотрудник: в MVP нет
платёжного шлюза, клиент переводит по реквизитам из `GET /bookings/requisites`.
