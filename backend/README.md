...
На Windows есть готовый батник `../scripts/start-backend.bat` — сам создаёт
venv, ставит зависимости, прогоняет миграции и стартует `uvicorn`. Ему всё
равно нужен `backend/.env` (шаг 3 ниже) и созданный админ (шаг 7) — сделайте
их один раз вручную, дальше можно запускать только батник.
...
## Бэкап и восстановление локальной БД

```
..\scripts\backup-db.bat              REM pg_dump -> scripts/backups/<db>_<дата>.backup
..\scripts\restore-db.bat [файл]      REM pg_restore --clean; без аргумента берёт самый свежий бэкап
```

Параметры подключения оба скрипта берут из `backend/.env` (`DATABASE_URL`),
ничего дополнительно указывать не нужно. `restore-db.bat` перед восстановлением
спрашивает подтверждение — он перезаписывает текущие данные в БД.
